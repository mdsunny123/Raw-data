...
# 0: SLC16A6+ Monocytes: SLC16A6
# 1: MARCO+ Macrophages: MARCO
# 2: IL1A+ M1 Macrophages: IL1A
cell_label = c("SLC16A6+ Monocytes", "MARCO+ Macrophages", "IL1A+ M1 Macrophages")
#################################################################################
...
VlnPlot(mydata, features=c("SLC16A6", "MARCO", "IL1A"), pt.size=0, cols=colors, ncol=3)+NoLegend()+theme(axis.title.x=element_blank())
...
set = c("CD80", "CD86", "IL12", "IL6", "TNFA", "CXCL9", "IL10", "TGFB1", "CD206", "CD209", "CD301")
DotPlot(mydata, features=set)+coord_flip()+scale_color_distiller(palette="RdYlBu")+theme_bw()+
theme(text=element_text(family="Times"), axis.text.x=element_text(angle=30, hjust=1, size=12, face="bold"), axis.text.y=element_text(face="bold", size=12), axis.title.x=element_blank(), axis.title.y=element_blank())
